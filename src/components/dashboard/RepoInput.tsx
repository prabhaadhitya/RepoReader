      ...
      <div className="w-full max-w-2xl mx-auto border-2 border-[#3F4450] bg-slate-900/40 backdrop-blur p-6 rounded-2xl space-y-6">
        ...
        <form onSubmit={handleAnalyze} className="space-y-4">
          <input
            ...
            disabled={loading}
            className="w-full p-3 rounded-lg bg-[#020817] border border-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
          />
        ...
        <div className="space-y-3" >
          ...
          <div className="flex flex-wrap gap-3">
            {examples.map((repo) => (
              <button 
                key={repo} type="button" disabled={loading} onClick={() => setRepoUrl(`https://github.com/${repo}`)}
                className="px-4 py-2 text-sm rounded-full bg-slate-800 hover:bg-slate-700 transition cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {repo}
              </button>
            ))}
